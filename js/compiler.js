...
function Compiler(source) {
	...
	this.checks = {}; // map<addr, list<check expressions>>
...
Compiler.prototype.reservedNames = {
	...
	":call": true, ":stringmode": true, ":assert": true, ":check": true,
};
...
Compiler.prototype.parseCheckTerminal = function (name) {
	// NUMBER | CONSTANT | LABEL | VREGISTER | '(' expression ')'
	var x = this.peek();
	if (x == 'PI') { return this.next(); }
	if (x == 'E') { return this.next(); }
	if (x == 'HERE') { this.next(); return this.hereaddr; }
	if (this.isRegister(x)) { this.next(); return "emulator.v[" + this.register(x) + "]"; }
	if (+x == +x) { return +this.next(); }
	if (x in this.constants) { return this.constants[this.next()]; }
	if (x in this.dict) { return this.dict[this.next()]; }
	if (x in this.protos) {
	...
	if (this.next() != '(') { throw "Undefined constant '" + x + "'."; }
	var value = this.parseCheck(name);
	if (this.next() != ')') { throw "Expected ')' for calculated constant '" + name + "'."; }
	return value;
}

Compiler.prototype.parseCheck = function (name) {
	// UNARY expression | terminal BINARY expression | terminal
	if (this.peek() == 'strlen') {
		return this.next(), ('' + this.next()).length;
	}
	if (this.peek() in unaryFunc) {
		return this.next() + ' ' + this.parseCheck(name);
	}
	var t = this.parseCheckTerminal(name);
	if (this.peek() in binaryFunc) {

		return t + ' ' + this.next() + ' ' + this.parseCheck(name);
	}
	else {
		return t;
	}
...
Compiler.prototype.parseChecked = function (name) {
	if (this.next() != '{') { throw "Expected '{' for check definition '" + name + "'."; }
	var value = this.parseCheck(name);
	if (this.next() != '}') { throw "Expected '}' for check definition '" + name + "'."; }
	return value;
}
...
Compiler.prototype.instruction = function (token) {
	if (token == ":") { this.resolveLabel(0); }
		...
		var message = this.peek() == '{' ? null : this.next();
		var value = this.parseChecked(message ? 'check ' + message : 'check');
		if (!value) { throw message ? "Check compile failed: " + message : "Check compile failed."; }

		if (this.here() in this.checks) {
			this.checks[this.here()].push(value);
		} else {
			this.checks[this.here()] = Array(value);
		}
	}
